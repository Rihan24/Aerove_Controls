...
class min_snap:
    def __init__(self, x, y, z, v,v_min,v_max,n=8):        
        ...
        b_y=np.append(b_y,np.array([-0.5,0.5,-0.5,0.5,0.5,-0.5]))#np.append(b_y,np.zeros(shape=(m-2)))
    ...
    def form_A_y(self,t):
        n = self.n
        m = self.m
        extra_A_part=np.zeros(shape=(m-1,n*m))
        for i in range(1,m):
            for j in range(i*n,(i+1)*n):
                indx=j-i*n
                extra_A_part[i-2][j]=indx*pow(t[i],indx-1)
        self.A_y=np.concatenate((self.A_y, extra_A_part), axis=0)
    ...
    def solve(self):
        ...
        self.p_y=solve_qp(self.Q, self.q,self.G,self.h, self.A_y, self.b_y)
    ...
    def cost_func(self,t_input):
        ...
        self.form_Q(t)
        self.form_A(t)
        self.A_y=np.copy(self.A)
        self.form_A_y(t)
        #print(self.n,self.m,np.shape(self.A),np.shape(self.A_y))
        #self.A_z=np.copy(self.A)
    ...
    def get_trajectory_var(self):
        ...
        #print(self.x_path,'\n\n',self.x_dot_path,'\n\n',self.x_dot_dot_path,'\n\n',self.y_path,'\n\n',self.y_dot_path,'\n\n',self.y_dot_dot_path,'\n\n',self.z_path,'\n\n',self.z_dot_path,'\n\n',self.z_dot_dot_path,'\n\n',self.psi_path,'\n\n')
        print(max(self.y_dot_dot_path),min(self.y_dot_dot_path))
        print(max(self.y_dot_path))
        # plt.figure(figsize=(10,5))
        # plt.plot(np.linspace(0,100,np.shape(self.y_dot_path)[0]),self.y_dot_path)
        # plt.show()
...
if __name__ == '__main__':
    ...
    y=[-35,-25,25,-25,25,-25]
    z=[7,7,7,7,7,7]
    x=[0,10,5,0,-5,-10,-5,0,5,10,5,0,-5,-10,-5,0]
    y=[0,12,20.66,22,20.667,12,3.334,2,3.334,12,20.66,22,20.667,12,3.334,2]
    z=[7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7]'''
